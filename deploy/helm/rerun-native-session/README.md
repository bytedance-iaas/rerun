...
前提:同 namespace 已部署 [`rerun-cloud`](../rerun-cloud/) chart(复用其凭证 Secret 与 APIG 网关)。

```sh
# 开一个会话(release 名 = 你的名字,小写字母/数字/中划线)。
# values 结构是 rerun-cloud chart 的子集:直接 -f 同一份 values 文件,
...
`rerun-cloud` 的 release 名不是 `rerun-cloud` 时,`secrets.existingSecret` 和 `ingress.className` 要按实际名字改。
