...
Catalog variant of `lerobot_dataset_review.py`. Same inputs, same output table —
the difference is where the summary numbers come from. The original buffers every
state/action vector in memory during ingest and computes from those lists. This
version keeps nothing: it flushes the recordings, registers them as a dataset, and
queries them back through the catalog.

    LeRobotDataset(repo, episodes=[0..4])   # downloads parquet + av1 video from HF
        -> log each episode to its own .rrd (cameras + 14-dim state/action), then flush
        -> register the .rrds as a Rerun DATASET
        -> read them back: episode lengths from segment_table(), per-episode
           metrics from reader(), one episode at a time
        -> create AND POPULATE a summary TABLE
        -> serve so the Viewer can browse all 5 episodes at the dataset level

Because the recordings are the source of truth, memory stays flat in episode count
(one episode is read back at a time) and `filter_contents` keeps the camera blobs
out of the query entirely.

Logging is unchanged from `lerobot_dataset_review.py`, so both scripts produce
interchangeable .rrd files.

Install:  pip install "rerun-sdk[datafusion]" lerobot pandas pyarrow
Run:      python lerobot_dataset_review_catalog.py
...
def episode_matrix(table, prefix):
    """Reassemble the `prefix/00`..`prefix/13` scalar columns into (n_frames, n_dims)."""
    cols = sorted(c for c in table.column_names if c.startswith(f"/{prefix}/"))
    if not cols:
        raise KeyError(f"No columns for /{prefix}. Columns: {table.column_names}")
    dims = [[v[0] if v else np.nan for v in table.column(c).to_pylist()] for c in cols]
    return np.asarray(dims, dtype=np.float64).T


def summarize(dataset, fps):
    """Build the per-episode summary by querying the catalog — replaces the buffers."""
    # Episode lengths come from dataset metadata, so no frame data is read. This is
    # an index extent, so a recording with gaps in `frame_index` would over-report.
    seg = dataset.segment_table().to_arrow_table()
    extent = {
        sid: int(end) - int(start) + 1
        for sid, start, end in zip(
            seg.column("rerun_segment_id").to_pylist(),
            seg.column(f"{INDEX}:start").to_pylist(),
            seg.column(f"{INDEX}:end").to_pylist(),
        )
    }
...
def main() -> None:
    ...
    for i in range(len(ds)):
        ...
        if ep not in recs:
            ...
            print(f"  -> episode {ep}")
        rec = recs[ep]
        rec.set_time(INDEX, sequence=fi)
    ...
    # Flush before registering, or the server may read a partially written .rrd.
    for rec in recs.values():
        rec.flush()
    print(f"Flushed {len(recs)} recordings to {OUT}/")

    # Register dataset, then build the summary by reading it back --------------
    ...
    df = summarize(client.get_dataset(DS), fps)
