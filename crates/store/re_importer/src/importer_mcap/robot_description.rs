        ...
        let msg = match msg {
            ...
            Err(err) if recover => {
                re_log::warn!(
                    "{}",
                    trf!(
                        "Stopping URDF scan at a truncated/corrupt MCAP tail: {err}",
                        "MCAP 尾部不完整或已损坏，停止扫描 URDF：{err}"
                    )
                );
                break;
            }
        ...
        match crate::importer_urdf::build_urdf_chunks_from_xml(
            ...
            Err(err) => {
                re_log::warn_once!(
                    "{}",
                    trf!(
                        "Failed to parse URDF from robot_description topic: {err}",
                        "从 robot_description 话题解析 URDF 失败：{err}"
                    )
                );
            }
        }
