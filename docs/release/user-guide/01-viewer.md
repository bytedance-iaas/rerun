...
`tos_endpoint` 和它是配合使用的:填 **rrd 缓存桶所在区域**的 endpoint,viewer 从这个域名自动识别区域并用它访问缓存桶,不需要也没有单独的 region 配置。
...
1. 向管理员申请创建一个会话,拿到会话专属的**域名**和**访问密码**。
...
> 所以打开时会**在线转换成 rerun 的 rrd 格式**再显示。
...
这些片段会**自动逐个流式载入,无需手动下载**;点其中一条即可查看它,也可让它优先加载。
...
- 在 **episode 行右键** →「Delete rrd artifact…」删这一段的缓存;在**数据集行右键** →「Delete all rrd artifacts…」删整个数据集的缓存。
...
- 只对**存放在 TOS 上的机器人(LeRobot v2 / v3)数据集**提供;不符合条件时,按钮不会出现。
